...
export const ANCHOR_PERIODS = [
  { id: 'before-work', label: '到公司之前', icon: '☀', hint: '起床后、洗漱、出门、通勤路上发生的事。' },
  { id: 'before-lunch', label: '午饭前', icon: '💻', hint: '上午工作中稳定发生的事。' },
  { id: 'midday', label: '中午', icon: '⏱', hint: '午饭、午休或中午固定流程里的事。' },
  { id: 'afternoon', label: '午后', icon: '☕', hint: '下午较稳定的会议、饮水、休息或切换动作。' },
  { id: 'leaving-work', label: '下班时', icon: '🚌', hint: '收尾、离开工位、通勤开始时发生的事。' },
  { id: 'after-work', label: '下班后', icon: '🏠', hint: '回家、晚饭、洗澡、家务或休闲前后的事。' },
  { id: 'before-bed', label: '睡前', icon: '☾', hint: '上床前和睡前固定发生的事。' },
];
...
export function canCreatePearlRecipe(selectedAnnoyance, selectedPearlCandidate, pearlTinyMethod, pearlTinyAction) {
  return Boolean(
    selectedAnnoyance.trim()
    && selectedPearlCandidate.trim()
    && pearlTinyMethod.trim()
    && pearlTinyAction.trim()
  );
}
