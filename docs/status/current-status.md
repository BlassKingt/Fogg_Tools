...
文档和管理体系已建立，并已改为中文主文档。核心整合已完成：根路由是工具箱首页，黄金行为探索器位于 `/golden-behavior`，能力链设计器已迁移到 `/ability-chain`，黄金行为可以继续进入突破设计。已保存功能 baseline，并完成第一轮统一视觉和导航打磨。针对第一轮视觉反馈，已修复默认蓝色下划线泄漏，并把全局背景从偏黄渐变收敛为更中性的暖灰纸面底。首页推荐路径已从词条块改为编号步骤说明，能力链 all-no 状态的“寻找新的黄金行为”已修为金色按钮。HTML 分析报告按钮已增加新标签页预览和页面内生成反馈。
...
1. 人工点击一次“生成分析报告”，确认新标签页预览和 HTML 下载体验。
2. 人工完整跑一次黄金行为结果页，复核“继续到突破设计”按钮观感。
3. 继续检查黄金行为焦点图在真实手机尺寸上的拖拽体验。
4. 做 Vercel 部署前检查。
