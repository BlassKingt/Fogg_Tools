...
文档和管理体系已建立，并已改为中文主文档。核心整合已完成：根路由是工具箱首页，黄金行为探索器位于 `/golden-behavior`，能力链设计器已迁移到 `/ability-chain`，黄金行为可以继续进入突破设计。已保存功能 baseline，并完成第一轮统一视觉和导航打磨。针对第一轮视觉反馈，已修复默认蓝色下划线泄漏，并把全局背景从偏黄渐变收敛为更中性的暖灰纸面底。首页推荐路径已从词条块改为编号步骤说明，能力链 all-no 状态的“寻找新的黄金行为”已修为金色按钮。HTML 分析报告按钮已增加新标签页预览和页面内生成反馈。黄金行为探索器已做移动端排布修正：候选行为在移动端改为列表卡片，焦点图保留原坐标系统并放入局部横向滚动，避免页面级横向溢出。
...
- `pages/golden-behavior.js` 的移动端样式避免压缩焦点图坐标，候选行为用列表形态保证阅读和点击稳定。
...
- 浏览器已验证 `/golden-behavior` 在 375px 移动端：初始页无页面级横向溢出，候选行为列表卡片无溢出，焦点图只在 `.focus-map` 内部横向滚动，页面本身无横向溢出。
...
3. 在真实手机上复核黄金行为焦点图的触控拖拽手感；当前自动化已验证布局不溢出，但真实触控手感仍建议人工确认。
4. 做 Vercel 部署前检查。
