...
- 锚点提示设计器已增加同一时段内锚点排序能力，支持拖动手柄和“上移/下移”按钮。
- 珍珠习惯页已加入蚌与石子/珍珠的渐进可视化；结果页珍珠习惯卡支持“擦亮珍珠”，打卡后珍珠发光。
- 习惯时间轴 7 个时段已加入日常语义符号，例如太阳、电脑、计时器、公交、月亮等。
- `npm run build` 已通过上述 bug 修复和体验增强后的版本；本地 `http://localhost:3002/anchor-prompts` 返回 200。当前 Codex in-app browser 自动控制连接不稳定，最终视觉细节建议人工刷新页面复核。
