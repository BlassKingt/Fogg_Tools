...
文档和管理体系已建立，并已改为中文主文档。核心整合已完成：根路由是工具箱首页，黄金行为探索器位于 `/golden-behavior`，能力链设计器已迁移到 `/ability-chain`，黄金行为可以继续进入突破设计。已保存功能 baseline，并完成第一轮统一视觉和导航打磨。针对第一轮视觉反馈，已修复默认蓝色下划线泄漏，并把全局背景从偏黄渐变收敛为更中性的暖灰纸面底。
