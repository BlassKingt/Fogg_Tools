...
- `pages/golden-behavior.js`：当前黄金行为探索器，含工具导航、localStorage 缓存、结果页重新寻找黄金行为入口，以及单个/多个黄金行为进入能力链的入口。移动端不要缩放焦点图坐标系统；候选行为使用独立环绕坐标，焦点图面板内部横向滚动，拖拽统一读取 mouse/touch 坐标。
